...
option.ROI                  = [10 10 320 240];  % A rectangle [x, y, w, h] that limits the processing area to ground locations.
...
option.timeWindowSize       = 20;               % A tuning parameter to specify the number of frames required to stabilize the confidence score of a track.
option.confidenceThresh     = 2;                % A threshold to determine if a track is true positive or false alarm.
option.ageThresh            = 3;                % A threshold to determine the minimum length required for a track being true positive.
option.visThresh            = 0.9;              % A threshold to determine the minimum visibility value for a track being true positive.
