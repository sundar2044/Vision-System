...
try
	vidObj = ipcam('http://192.168.127.50/video','admin','admin'); %Default; You can tweak webcam properties
    %vidObj = webcam(2);
...
global storeName;
storeName={};
while double(get(fdrFig,'currentCharacter')) ~= 27 && count<8 && ~isDone 
	bestGuess = '?';
    saveBestGuess=[];
    count = count+1;
	RGBFrame = snapshot(vidObj);
	...
	for jj = 1:size(bboxes,1)%#ok
		if all(bboxes(jj,3:4) >= QC.minBBSize)
			...
			if bestGuess == 0
                ...
                storeName = [storeName, bestGuess];
%                 fprintf(fid,bestGuess );
%                 fprintf(fid,'\n' );
			end
			%tPredict = toc
			RGBFrame = 	insertObjectAnnotation(RGBFrame, 'rectangle', bboxes(jj,:), bestGuess,'FontSize',30);
		end
    end
...
%% Finds most repeated name for authentication
y = unique(storeName);
n = zeros(length(y), 1);
for iy = 1:length(y)
  n(iy) = length(find(strcmp(y{iy}, storeName)));
end
[~, itemp] = max(n);
mostRepeatedName=y(itemp);
msgbox(mostRepeatedName,'Welcome');
