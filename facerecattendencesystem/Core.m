...
% Core MATLAB code for Core.fig
%      Core, by itself, creates a new Core or raises the existing
%      singleton*.
%
%      H = Core returns the handle to a new Core or the handle to
%      the existing singleton*.
%
%      Core('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in Core.M with the given input arguments.
%
%      Core('Property','Value',...) creates a new Core or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Core_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Core_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Core

% Last Modified by GUIDE v2.5 20-Jul-2016 08:11:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Core_OpeningFcn, ...
    'gui_OutputFcn',  @Core_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Core is made visible.
function Core_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Core (see VARARGIN)

% Choose default command line output for Core
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Core wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Core_OutputFcn(hObject, eventdata, handles)
